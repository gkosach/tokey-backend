...
export const getProperties = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (latitude && longitude) {
      ...
      const degreesPerKm = 0.009; // 1km ≈ 0.009 градусов

      whereConditions.push(Prisma.sql`
    l.latitude BETWEEN ${lat - radiusInKm * degreesPerKm} 
    AND ${lat + radiusInKm * degreesPerKm}
    AND l.longitude BETWEEN ${lng - radiusInKm * degreesPerKm} 
    AND ${lng + radiusInKm * degreesPerKm}
  `);
    }
